...
import { CategoryKey, FortuneContent, EconomicsContent, HealthContent, QuizContent, LanguageContent } from '../types';
...
const LanguageChallenge: React.FC<{ content: LanguageContent, onComplete: () => void }> = ({ content, onComplete }) => (
     <div className="space-y-6">
        <div className="bg-blue-50 p-6 rounded-xl border border-blue-200 text-center">
            <p className="text-sm text-blue-600 mb-2">{content.context}</p>
            <p className="text-2xl font-bold text-blue-900">"{content.phrase}"</p>
            <p className="text-lg text-gray-600 mt-2">{content.translation}</p>
        </div>
        <button onClick={onComplete} className="w-full bg-indigo-600 text-white py-3 rounded-lg hover:bg-indigo-700 transition font-bold text-lg">
            완료
        </button>
...
const ChallengePage = () => {
    ...
    const { completeChallenge, userInfo } = useAAppContext();
    ...
    const renderContent = () => {
        ...
        switch (categoryKey) {
            ...
            case 'language':
                return <LanguageChallenge content={content as LanguageContent} onComplete={handleComplete} />;
            default:
